...
def run_quarterly_enmods_water_quality_dag():

    @task(
        executor_config=generate_executor_config_template('largest'),
        task_id="quarterly_enmods_water_quality"
    )
    def run_quarterly_enmods_water_quality(**kwargs):
        ...
        conn = hook.get_conn()
        enmods_quarterly_scraper = QuarterlyEnmodsArchiveUpdatePipeline(date_now=logical_time, db_conn=conn, quarterly=True)

        enmods_quarterly_scraper.download_data()
        enmods_quarterly_scraper.transform_data()
        enmods_quarterly_scraper.clean_up()

    run_quarterly_enmods_water_quality()

run_quarterly_enmods_water_quality = run_quarterly_enmods_water_quality_dag()
